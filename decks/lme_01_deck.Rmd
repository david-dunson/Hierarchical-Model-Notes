...
```{r catplot1, echo=FALSE,warning=FALSE}
library(sjPlot)
plot_model(m1,sort.est=TRUE)
```
...
##
```{r ranef2, echo=FALSE, warning=FALSE}
library(lme4)
m2=lmer(mathscore~(1|school),data=nels)
summary(m2)
library(sjstats)
...
How do we conduct a formal test of heterogeneity in this random effects setting?  Well, this is a bit more complicated than in the fixed effects setting, and we'll return to the issue in a couple of lectures. For now just note that the school-level estimated variance is not small.
