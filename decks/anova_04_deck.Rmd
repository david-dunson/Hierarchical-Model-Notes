...
&\propto & p(\mu \mid \tau^2) p(\tau^2)p(\sigma^2)\prod_{j=1}^J p(\mu_j \mid \mu, \tau^2)\prod_{i=1}^{n_j} p(y_{ij} \mid \mu_j, \sigma^2) \\
&\propto& p(\mu_j \mid \mu, \tau^2) \prod_{j=1}^{n_j} p(y_{ij} \mid \mu_j, \sigma^2) ~~~~ \propto ~~~~ \exp \left\{-\frac{(\mu_j-\mu)^2}{2\tau^2}\right\} \exp \left\{-\frac{\sum (y_{ij}-\mu)^2}{2\sigma^2}\right\}
\end{eqnarray*}
...
```{r standefault, cache=TRUE, warning=FALSE, echo=FALSE}
load("~/Documents/GitHub/STA-410-610/PsychBikeData")
library(rstanarm)
M1_stanlmer=stan_lmer(`passing distance` ~ (1 | kerb),  data = PsychBikeData)

```

## print output
```{r standefaultprior, cache=TRUE}
prior_summary(object=M1_stanlmer)

```

## print output
```{r standefaultoutput, cache=TRUE}
