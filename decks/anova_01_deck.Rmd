...
```{r descstat}
load("~/Documents/GitHub/STA-410-610/PsychBikeData")
...
\end{eqnarray*}
where $\mu_{j}=\mu+\alpha_j$. These two equations are simply alternate parameterizations of the same model. In each case, we estimate a separate mean passing distance $\mu_j$ as a function of each of the 5 curb distances tested.
...
Using algebra, you can show that $E(MSG)=\sigma^2 + \frac{\sum n_j (\mu_j-\mu)^2}{J-1}$. Under the null hypothesis that all the group means are 0, this expectation reduces to $\sigma^2$.
